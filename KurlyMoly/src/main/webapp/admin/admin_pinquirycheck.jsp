...
<jsp:useBean id="pinquiryDAO" class="com.kurly.inquiry.PinquiryDAO"/>
<c:set var="pinquiryList" value ="${pinquiryDAO.selectList()}"/>
<c:set var="noreplyList" value ="${pinquiryDAO.noreplyList()}"/>
<c:set var="replyList" value ="${pinquiryDAO.replyList()}"/>
...
<%  
		...
		Connection conn = null;
         ...
         try{
            ...
            if(conn != null){
               sql = "select count(piq_idx) as total from tb_pinquiry;";
               ...
               sql = "select * from tb_pinquiry order by piq_idx desc limit ?, ?";
               pstmt = conn.prepareStatement(sql);
   ...
   function chageLangSelect(){
      ...
      <script>
		...
		function chageLangSelect(){
	    	...
	    	window.console.log(selectValue);
	    	var pageNumJs = <%=pageNum%>;
	    	location.href="./admin_pinquirycheck.jsp?pageNum=" + pageNumJs + "&pageSize=" + selectValue;
		}
	</script>
            ...
            <div class="side_menu">
                ...
                <div id="tab6" class="menutab __activepage">
                    ...
                    <ul>
                        ...
                        <li class="__activepage1">상품 문의</li>
...
<!-- 
                <div class="filter">
                    <form class="regForm" method='GET' action='admin_pinquirycheck_ok.jsp'>
                            ...
                            <li class="search_detail clear">
                                ...
                                <div class="search_box"> 
                                    <select name="select_search">
                                        ...
                                        <option value="상품문의번호">상품문의번호</option>
 ...
 -->
                <div class="listview">
                    <div class="subtitle float_left">
                        <h3>상품 문의 목록 (총 <span>${fn:length(pinquiryList)}</span> 개)</h3>
                    </div>
                    ...
                    <div class="bssBox">
                        <table class="bbsListTbl">
                            <tr>
                             ...
                             <c:forEach var="item" items="${pinquiryList}" varStatus ="status"  begin="<%=start %>" end="<%=start+pagePerCount-1 %>" >
	                            <tr>

									<td><a href="a./admin_pinquirydetail.jsp?piq_idx=${item.idx}" >${item.idx}</a></td>
									<td>${item.prodidx}</td>
									<td>${item.title}</td>
									<td>${item.userid}</td>
									<td>${item.issecret}</td>
									...
									<td>${item.isreply}</td>
									<td><input type="button" class="bbs_btn" value="답변" onclick="location.href='./admin_pinquirydetail.jsp?piq_idx=${item.idx}'" ></td>
								</tr>
							</c:forEach>
                      ...
                      <div  class = "pagination">
						...
						<%
						       for(int i=1; i<=pageNums; i++) {
						             out.print("<a href='admin_pinquirycheck.jsp?pageNum=" + i + "&pageSize=" + 
